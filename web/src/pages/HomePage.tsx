...
export const HomePage: React.FC<Props> = ({ onSelectPool }) => {
  ...
  return (
    <div className="min-h-screen bg-monad-bg bg-grid-pattern flex flex-col font-pixel">
      ...
      {/* ── Card Packs ──────────────────────────────────────────────────────── */}
      <div className="flex-1 flex items-start justify-center px-4 md:px-8 py-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-3xl">
          {pools?.map((pool, idx) => {
            ...
            return (
              <div
                key={pool.id}
                className={`relative bg-gradient-to-br ${art.bg} border-4 ${art.border} ${art.glow} p-6 flex flex-col gap-4`}
              >
                {/* Pool badge */}
          ...
          {!pools && (
            <>
              {[0, 1].map((i) => (
                <div key={i} className="border-4 border-gray-700 bg-gray-900 p-6 animate-pulse">
